...
public class UserManager {
    ...
    public String changeUserRole(int userId, Role newRole) {
        synchronized (usersLock) {
            List<User> users = loadUsers();
            for (User user : users) {
                if (user.getUserId() == userId) {
                    // setRole is private on User, so we use the package-accessible
                    // changeRole method added to User for this purpose.
                    user.changeRole(newRole);
                    saveUsers(users);
                    return "Role of user '" + user.getUserName() + "' changed to " + newRole + ".";
                }
            }
